...
# Certifique-se de manter a sua URL e KEY corretas aqui
...
# --- CONTROLE DE TELAS (LOGIN OU SISTEMA) ---
if not st.session_state['logado']:
    ...
    with col_b2:
        ...
        usuario_input = st.text_input("Usuário", key="txt_user_final")
        senha_input = st.text_input("Senha", type="password", key="txt_pass_final")
        
        if st.button("Entrar", use_container_width=True, key="btn_entrar_final"):
            if usuario_input.strip().lower() == "admin" and senha_input.strip() == "123":
    ...
    # EXIBE O SISTEMA APENAS SE ESTIVER LOGADO (MUDANÇA ESTRUTURAL DE SEGURANÇA)
